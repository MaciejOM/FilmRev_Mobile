...
export default function FilmDetail() {
  ...
  useEffect(() => {
    const addSubscription = DeviceEventEmitter.addListener("newReviewAdded", (newReview) => {
      setReviews((prev) => [newReview, ...prev]);
    });
    
    const editSubscription = DeviceEventEmitter.addListener("reviewEdited", (updatedData) => {
      setReviews((prev) =>
        prev.map((r) => r.id === updatedData.reviewId ? { ...r, ocena: updatedData.rating, tresc: updatedData.comment, tags: updatedData.selectedTags, isEdited: true } : r)
      );
    });

    const likeSubscription = DeviceEventEmitter.addListener("reviewLikeToggled", (data) => {
      setReviews((prev) =>
        prev.map((r) => r.id === data.reviewId ? { ...r, likes: data.newLikes } : r)
      );
    });

    const deleteSubscription = DeviceEventEmitter.addListener("reviewDeleted", (data) => {
      setReviews((prev) => prev.filter((r) => r.id !== data.reviewId));
    });

    const avatarSubscription = DeviceEventEmitter.addListener("avatarChanged", (data) => {
      setReviews((prev) =>
        prev.map((r) => r.userId === data.userId ? { ...r, avatar: data.newAvatar } : r)
      );
    });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (userAuth) {
        ...
        const qLists = query(collection(db, "custom_lists"), where("userId", "==", userAuth.uid));
        customListsPromise = getDocs(qLists);
      }

      const [fetchedReviews, movieSnap, userDocSnap, snapLists] = await Promise.all([
        reviewsPromise,
        movieDocPromise,
        userDocPromise,
        customListsPromise
      ...
      if (userAuth && snapLists) {
        setMyCustomLists(snapLists.docs.map((d: any) => ({ id: d.id, ...d.data() })));
      }
      ...
      { text: "Anuluj", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert("Błąd", "Brak połączenia. Nie udało się usunąć recenzji.");
          }
        },
      },
    ]);
  ...
  const toggleCustomListMembership = async (listId: string, currentItems: string[]) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    const listRef = doc(db, "custom_lists", listId);
    const exists = currentItems.includes(documentId);

    const previousLists = [...myCustomLists];
    setMyCustomLists(prev => prev.map(l => l.id === listId ? { ...l, items: exists ? l.items.filter((i: string) => i !== documentId) : [...l.items, documentId] } : l));
    ...
    try {
      ...
      Alert.alert("Błąd", "Nie udało się zaktualizować listy niestandardowej. Sprawdź połączenie z siecią.");
    }
  };
  ...
  const handleToggleFavourite = async () => {
    ...
    try {
      const result = await toggleUserList(currentUser, "favourites", documentId);
  ...
  const handleLikePress = async (reviewId: string, currentLikes: string[] = []) => {
    ...
    try {
      ...
      setReviews(previousReviews);
      DeviceEventEmitter.emit("reviewLikeToggled", { reviewId, newLikes: currentLikes }); 
      Alert.alert("Błąd", "Nie udało się polubić recenzji.");
    }
  };

  const totalReviews = reviews.length;
  const averageRating = totalReviews > 0 ? (reviews.reduce((sum, r) => sum + r.ocena, 0) / totalReviews).toFixed(1) : 0;
          ...
          <View style={{ paddingHorizontal: 20 }}>
            <View style={styles.reviewCard}>
              <View style={styles.reviewHeader}>
                {rev.avatar ? <Image source={{ uri: rev.avatar }} style={styles.reviewAvatar} transition={200} /> : (
                  <View style={styles.reviewAvatarPlaceholder}>
                    <Text style={{ color: "white" }}>{rev.nazwa_uzytkownika?.charAt(0).toUpperCase() || "?"}</Text>
                  </View>
                )}
                <TouchableOpacity onPress={() => currentUser && currentUser === rev.userId ? router.push("/Profile") : router.push({ pathname: '/OtherProfile', params: { userId: rev.userId } })} style={{ flex: 1, marginRight: 10 }}>
                  <Text style={styles.reviewAuthor} numberOfLines={1}>{rev.nazwa_uzytkownika}</Text>
                </TouchableOpacity>
                {rev.isEdited && <Text style={{ color: AppColors.textGray, fontSize: 12, marginRight: 10, fontStyle: "italic" }}>edytowano</Text>}
                <View style={styles.reviewScoreContainer}>
                  <Text style={styles.reviewScoreText}>{rev.ocena}/5 </Text>
                  <View style={{ flexDirection: "row" }}>
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Text key={star} style={star <= rev.ocena ? styles.starSelectedSmall : styles.starUnselectedSmall}>★</Text>
                    ))}
                  </View>
                </View>
              </View>
              ...
              {rev.tags && rev.tags.length > 0 && (
                <View style={styles.reviewTagsContainer}>
                  {rev.tags.map((tag: string) => (
                    <View key={tag} style={styles.reviewTagBadge}><Text style={styles.reviewTagText}>{tag}</Text></View>
                  ))}
                </View>
              )}
              ...
              <View style={styles.reviewFooter}>
                <View style={styles.reviewActionsLeft}>
                  {currentUser && currentUser === rev.userId && (
                    <>
                      <TouchableOpacity style={[styles.deleteButton, { marginTop: 0, backgroundColor: "rgba(255, 255, 255, 0.1)" }]} onPress={() => router.push({ pathname: "/EditReview", params: { reviewId: rev.id, movieId: documentId, initialRating: rev.ocena, initialComment: rev.tresc, initialTags: JSON.stringify(rev.tags || []) } })}><MaterialCommunityIcons name="pencil" size={24} color="white" /></TouchableOpacity>
                      <TouchableOpacity style={[styles.deleteButton, { marginTop: 0 }]} onPress={() => handleDeleteReview(rev.id)}><MaterialCommunityIcons name="trash-can" size={24} color={AppColors.buttonDanger} /></TouchableOpacity>
                    </>
                  )}
                </View>
                {currentUser && (
                  <TouchableOpacity style={styles.likeButton} onPress={() => handleLikePress(rev.id, rev.likes || [])}>
                    <MaterialCommunityIcons name={rev.likes?.includes(currentUser) ? "cards-heart" : "cards-heart-outline"} size={24} color={rev.likes?.includes(currentUser) ? AppColors.primary : AppColors.textGray} />
                    <Text style={styles.likeCountText}>{rev.likes ? rev.likes.length : 0}</Text>
                  </TouchableOpacity>
                )}
              </View>
            </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setIsListModalVisible(false)}><MaterialIcons name="close" size={26} color="white" /></TouchableOpacity>
            </View>
            <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 10 }}>
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  toggleUserList(currentUser!, "watchlist", documentId)
                    .catch(() => {
                      setIsInWatchlist(!newState);
                      DeviceEventEmitter.emit("watchlistToggled", {
                        mediaId: documentId,
                      ...
                      Alert.alert("Błąd", "Brak sieci.");
                    });
                }}
              >
                <MaterialIcons name={isInWatchlist ? "check-box" : "check-box-outline-blank"} size={24} color={isInWatchlist ? AppColors.primary : "#999"} />
              ...
              toggleUserList(currentUser!, "watched" as any, documentId)
                .catch(() => {
                  setIsInWatched(!newState);
                  DeviceEventEmitter.emit("watchedToggled", {
                    mediaId: documentId,
              ...
              >
                <MaterialIcons name={isInWatched ? "check-box" : "check-box-outline-blank"} size={24} color={isInWatched ? AppColors.primary : "#999"} />
                <Text style={styles.listItemText}>Obejrzane</Text>
              </TouchableOpacity>
              
              <View style={{ height: 1, backgroundColor: "#444", marginVertical: 5 }} />
              {myCustomLists.length === 0 ? <Text style={styles.noCustomListsText}>Brak utworzonych list niestandardowych.</Text> : myCustomLists.map((list) => { const isChecked = list.items?.includes(documentId) || false; return ( <TouchableOpacity key={list.id} style={styles.listItemRow} onPress={() => toggleCustomListMembership(list.id, list.items || [])}><MaterialIcons name={isChecked ? "check-box" : "check-box-outline-blank"} size={24} color={isChecked ? AppColors.primary : "#999"} /><Text style={styles.listItemText}>{list.name}</Text></TouchableOpacity> ); })}
            </ScrollView>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  errorText: { color: "#ff6b6b", textAlign: "center", marginBottom: 10, fontSize: 16 },
  ...
  deleteButton: { alignSelf: "flex-start", paddingVertical: 5, paddingHorizontal: 10, backgroundColor: "rgba(255, 255, 255, 0.1)", borderRadius: 5 },
  reviewFooter: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 15, borderTopWidth: 1, borderColor: "rgba(255,255,255,0.05)", paddingTop: 10 },
  reviewActionsLeft: { flexDirection: "row", gap: 10 },
  likeButton: { flexDirection: "row", alignItems: "center", gap: 6, padding: 5 },
  likeCountText: { color: "white", fontSize: 14, fontWeight: "bold" },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" },
  modalContent: { backgroundColor: "#3a3c4f", borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 20, minHeight: 250, borderTopWidth: 1, borderColor: "#555" },
  modalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20 },
  modalTitle: { color: "white", fontSize: 18, fontWeight: "bold" },
  listItemRow: { flexDirection: "row", alignItems: "center", paddingVertical: 12, gap: 12 },
  listItemText: { color: "white", fontSize: 16, fontWeight: "500" },
  noCustomListsText: { color: "#999", fontStyle: "italic", textAlign: "center", marginTop: 10 },
  reviewTagsContainer: { flexDirection: "row", flexWrap: "wrap", marginBottom: 8, gap: 6 },
  reviewTagBadge: { backgroundColor: "rgba(255, 255, 255, 0.1)", paddingVertical: 3, paddingHorizontal: 8, borderRadius: 12, borderWidth: 1, borderColor: "rgba(255, 255, 255, 0.2)" },
  reviewTagText: { color: "#ccc", fontSize: 10, fontWeight: "bold", textTransform: "uppercase" },
});