...
import { Alert, DeviceEventEmitter, FlatList, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";

import { useAuth } from "@/hooks/AuthContext";
import { deleteCustomList, getCustomListDetails, renameCustomList } from "@/hooks/firebaseDatabase";

export default function CustomList() {
  const { listId, creatorId } = useLocalSearchParams<{ listId: string; creatorId: string }>();
  const { user } = useAuth();
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setError("Brak połączenia z siecią. Nie udało się załadować danych listy.");
    } finally {
      setIsLoading(false);
    }
  }, [listId]);
  ...
  const handleSaveName = async () => {
    ...
    try {
      const netState = await NetInfo.fetch();
      if (!netState.isConnected) {
        Alert.alert("Brak sieci", "Połącz się z internetem, aby zmienić nazwę listy.");
        return;
      }

      await renameCustomList(listId, newListName.trim());
      setListDetails((prev: any) => ({ ...prev, name: newListName.trim() }));
      DeviceEventEmitter.emit("customListRenamed", { listId, newName: newListName.trim() });
  ...
  const handleDeleteList = () => {
    Alert.alert("Usuwanie listy", "Czy na pewno chcesz bezpowrotnie usunąć tę listę?", [
      { text: "Anuluj", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            const netState = await NetInfo.fetch();
            if (!netState.isConnected) {
              Alert.alert("Brak sieci", "Połącz się z internetem, aby usunąć listę.");
              return;
            }
  ...
  const dataToShow = useMemo(() => {
    return activeTab === "movie" ? (listDetails?.movies || []) : (listDetails?.tv || []);
  ...
  const renderGridItem = useCallback(({ item }: { item: any }) => {
    ...
    const cleanId = item.tmdb_id || item.id?.toString().replace("movie_", "").replace("tv_", "");
  ...
  if (isLoading) {
    return (
      <View style={globalStyles.container}>
        ...
        <FlatList data={[1,2,3]} numColumns={numGridColumns} columnWrapperStyle={{ gap: 15 }} renderItem={() => <Skeleton style={[styles.gridItem, { width: itemWidth }]} />} />
      </View>
    );
  }
  ...
  if (!listDetails) {
    return (
      <View style={globalStyles.centerContainer}>
        ...
        <TouchableOpacity onPress={() => router.back()}><Text style={{color: AppColors.primary, marginTop: 10}}>Wróć</Text></TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={globalStyles.container}>
      <View style={styles.customHeader}>
        ...
        {isOwner && (
          isEditingName ? (
            <TouchableOpacity onPress={handleSaveName} style={styles.actionIconButton}>
              <MaterialIcons name="check" size={26} color={AppColors.primary} />
            </TouchableOpacity>
          ) : (
            <TouchableOpacity onPress={() => setIsEditingName(true)} style={styles.actionIconButton}>
      ...
      <View style={{ flex: 1 }}>
        <View style={styles.tabsContainer}>
          <TouchableOpacity style={[styles.tabButton, activeTab === "movie" && styles.tabButtonActive]} onPress={() => setActiveTab("movie")}>
            <Text style={[styles.tabText, activeTab === "movie" && styles.tabTextActive]}>Filmy ({listDetails.movies?.length || 0})</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.tabButton, activeTab === "tv" && styles.tabButtonActive]} onPress={() => setActiveTab("tv")}>
            <Text style={[styles.tabText, activeTab === "tv" && styles.tabTextActive]}>Seriale ({listDetails.tv?.length || 0})</Text>
          </TouchableOpacity>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={globalStyles.emptyText}>Lista jest pusta. Użyj wyszukiwarki, aby dodać tytuły!</Text>}
...
const styles = StyleSheet.create({
  ...
  actionIconButton: { padding: 6, justifyContent: "center", alignItems: "center" },
  deleteListButton: { padding: 6, justifyContent: "center", alignItems: "center", marginLeft: 6 },
