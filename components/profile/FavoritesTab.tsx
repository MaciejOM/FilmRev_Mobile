...
const FavoritesTab = ({ data }: FavoritesTabProps) => {
  ...
  const { movies, tvShows } = useMemo(() => {
    ...
    return {
      movies: data.filter((item) => item.typ === "movie" || item.type === "movie"),
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>Nie masz jeszcze żadnych zapisanych tytułów.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabsContainer}>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === "movie" && styles.tabButtonActive]}
          onPress={() => setActiveTab("movie")}
        >
          <Text style={[styles.tabText, activeTab === "movie" && styles.tabTextActive]}>
            Filmy ({movies.length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === "tv" && styles.tabButtonActive]}
          onPress={() => setActiveTab("tv")}
        >
          <Text style={[styles.tabText, activeTab === "tv" && styles.tabTextActive]}>
      ...
      {dataToShow.length === 0 ? (
        <Text style={styles.emptyTextCategory}>Brak tytułów w tej kategorii.</Text>
      ) : (
...
const styles = StyleSheet.create({
  ...
  emptyTextCategory: { color: AppColors.textGray, fontSize: 14, fontStyle: "italic", marginTop: 5 },
  tabsContainer: { flexDirection: "row", marginTop: 5, marginBottom: 15, gap: 15 },
  ...
  gridItem: { aspectRatio: 2 / 3, backgroundColor: "#3a3c4f", borderRadius: 8, overflow: "hidden", marginBottom: 5 },
  posterImage: { width: "100%", height: "100%" },
  placeholder: { flex: 1, justifyContent: "center", alignItems: "center", padding: 8 },
  placeholderText: { color: "white", fontSize: 12, textAlign: "center", fontWeight: "bold" },
});