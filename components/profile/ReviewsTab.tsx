...
const ReviewCard = memo(({ rev, isReadOnly, onDelete, onLike }: {
  rev: any;
  isReadOnly: boolean;
            ...
            if (rev.movieData) {
              router.push({
                ...
                params: {
                  ...
                  gatunki: rev.movieData.gatunki ? rev.movieData.gatunki.join(", ") : "",
        ...
        <View style={styles.reviewScoreContainer}>
          ...
          <View style={{ flexDirection: "row" }}>
            {[1, 2, 3, 4, 5].map((star) => (
              <Text
                key={star}
                style={star <= rev.ocena ? styles.starSelectedSmall : styles.starUnselectedSmall}
              >
                ★
              </Text>
            ))}
      ...
      <View style={styles.reviewFooter}>
        <View style={styles.reviewActionsLeft}>
          {!isReadOnly && (
            <>
              <TouchableOpacity
                style={[styles.deleteButton, { backgroundColor: "rgba(255, 255, 255, 0.1)" }]}
                onPress={() =>
                  router.push({
                    pathname: "/EditReview",
                    params: {
        ...
        >
          <MaterialCommunityIcons
            name={rev.likes?.includes(currentUid as string) ? "cards-heart" : "cards-heart-outline"}
            size={24}
            color={rev.likes?.includes(currentUid as string) ? AppColors.primary : AppColors.textGray}
          />
...
const ReviewsTab = ({ data, isReadOnly = false, onDeleteReview }: ReviewsTabProps) => {
  ...
  useEffect(() => {
    const likeSub = DeviceEventEmitter.addListener("reviewLikeToggled", (eventData) => {
      setReviewsList((prev) =>
        prev.map((rev) =>
          rev.id === eventData.reviewId ? { ...rev, likes: eventData.newLikes } : rev
        )
      );
    });
  ...
  const handleLikePress = useCallback(async (reviewId: string, currentLikes: string[] = []) => {
    ...
    setReviewsList((prev) =>
      prev.map((rev) => (rev.id === reviewId ? { ...rev, likes: newLikes } : rev))
    );
    ...
    try {
      ...
      setReviewsList((prev) =>
        prev.map((rev) => (rev.id === reviewId ? { ...rev, likes: currentLikes } : rev))
      );
      DeviceEventEmitter.emit("reviewLikeToggled", { reviewId, newLikes: currentLikes });
      Alert.alert("Błąd", "Brak połączenia z siecią. Polubienie nie zostało zapisane.");
    }
  }, []);
...
const styles = StyleSheet.create({
  ...
  reviewTargetTitle: { color: "white", fontWeight: "bold", fontSize: 16, textDecorationLine: "underline" },
  editedText: { color: AppColors.textGray, fontSize: 10, marginRight: 10, fontStyle: "italic" },
  ...
  deleteButton: { paddingVertical: 5, paddingHorizontal: 10, backgroundColor: "rgba(228, 48, 87, 0.52)", borderRadius: 5 },
